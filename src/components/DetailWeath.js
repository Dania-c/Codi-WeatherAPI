...
const DetailWeath =(props) => {
    const key = '047c54c9be210c32c9b4c6c654079e73';
    ...
    const city="Tripoli"  ;
    useEffect(() => {
        fetch(`http://api.openweathermap.org/data/2.5/forecast?q=${city}&cnt=8&units=metric&appid=${key}`)
            .then(res => res.json())
            .then(data => {
                console.log(data);
                var i = 0;
